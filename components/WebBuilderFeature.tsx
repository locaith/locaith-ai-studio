...
export const WebBuilderFeature: React.FC<WebBuilderFeatureProps> = ({
    ...
    const saveProject = async (code: string, msgs: Message[]) => {
        try {
            ...
            const projectData = {
                user_id: user.id, // CRITICAL: Must have user_id
            ...
            if (projectId) {
                ...
                delete (updateData as any).subdomain;
                ...
                // Create new - must include user_id
                ({ data, error } = await supabase
                    .from('websites')
                    .insert([projectData])
                    .select()
            ...
            if (data) {
                ...
                console.log('✅ Project saved:', data.id, 'user:', user.id);

                // Log activity
                trackActivity({
                    feature_type: 'web-builder',
                    action_type: projectId ? 'update' : 'create',
                    action_details: {
                        project_id: data.id,
