-- FIX SCHEMA AND POLICIES V5 (CASCADE FIX)
-- Mục tiêu: Sửa lỗi "cannot drop function... depend on it" bằng cách dùng CASCADE để xóa sạch các ràng buộc cũ.
-- Hướng dẫn: Copy toàn bộ và chạy trong Supabase SQL Editor.

BEGIN;

-- 1. Xóa hàm cũ và TOÀN BỘ các policy phụ thuộc (CASCADE là chìa khóa)
...
-- 2. Dọn dẹp bảng (Ngắt liên kết và xóa bảng cũ)
...
-- 3. Tạo lại bảng GROUPS (Chuẩn UUID)
...
-- 4. Tạo lại bảng GROUP_MEMBERS (Chuẩn UUID)
...
-- 5. Khôi phục cột group_id cho MESSAGES và APPOINTMENTS
...
-- 7. Tạo lại hàm helper (SECURITY DEFINER)
...
-- === MESSAGES ===
-- Xóa policy cũ nếu còn sót lại sau khi drop function cascade
